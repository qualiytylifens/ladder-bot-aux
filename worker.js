...
const WORKER_ENABLED = String(process.env.WORKER_ENABLED || "true").toLowerCase() === "true" ||
...
async function processJob(job) {
  ...
  if (!resp.ok) {
    ...
    log({ level: "error", msg: "webhook_failed", job_id, status: resp.status, body: resp.text?.slice(0, 500) });
